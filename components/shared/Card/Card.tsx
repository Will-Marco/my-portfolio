...
export default function CardItem({ card }: { card: CardsType }) {
  ...
  return (
    <Card className="w-fit">
      <CardHeader
        className="w-full h-96 flex flex-col justify-between cursor-pointer"
      ...
      >
        ...
        />
        <div>
          <CardTitle className="pt-6 pb-2 text-xl lg:text-3xl border-t-2">
            {card.title}
          </CardTitle>
      ...
      <CardContent className="w-full flex justify-between items-center">
        <p className="text-sm lg:text-base xl:text-xl font-semibold text-slate-400">{card.position}</p>
        <Button
          variant="ghost"
      ...
      <CardFooter className="h-28 felx flex-wrap content-center gap-2">
        {card.techStack.map((tech: string) => (
          <p
            key={tech}
            className="px-2 py-1 bg-slate-900 uppercase text-[10px] md:text-[10px] lg:text-xs text-slate-500 font-semibold rounded-md border border-slate-500 "
          >
            #{tech}
          </p>
        ))}
